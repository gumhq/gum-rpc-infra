...
use plerkle_messenger::{
    redis_messenger::RedisMessenger, ConsumptionType, Messenger, MessengerConfig, RecvData,
    ACCOUNT_STREAM, TRANSACTION_STREAM,
};
            ...
            let result = tokio::spawn(async {
                ...
                loop {
                    if let Ok(data) = messenger
                        .recv(TRANSACTION_STREAM, ConsumptionType::New)
                        .await
                    {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let result = tokio::spawn(async {
                ...
                loop {
                    if let Ok(data) = messenger.recv(ACCOUNT_STREAM, ConsumptionType::New).await {
